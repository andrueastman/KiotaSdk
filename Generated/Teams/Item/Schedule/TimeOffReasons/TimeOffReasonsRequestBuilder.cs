...
using GraphSdk.Teams.Item.Schedule.TimeOffReasons.Item;
...
namespace GraphSdk.Teams.Item.Schedule.TimeOffReasons {
    ...
    public class TimeOffReasonsRequestBuilder {
        ...
        /// <summary>Gets an item from the GraphSdk.teams.item.schedule.timeOffReasons.item collection</summary>
        ...
        public TimeOffReasonsRequestBuilder(Dictionary<string, object> pathParameters, IRequestAdapter requestAdapter) {
            ...
            UrlTemplate = "https://graph.microsoft.com/v1.0/teams/{team_id}/schedule/timeOffReasons{?top,skip,search,filter,count,orderby,select}";
            var urlTplParams = new Dictionary<string, object>(pathParameters);
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<TimeOffReason> PostAsync(TimeOffReason body, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default) {
            _ = body ?? throw new ArgumentNullException(nameof(body));
